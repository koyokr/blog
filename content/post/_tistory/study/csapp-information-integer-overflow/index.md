...
$$x$$, $$y$$가 int형일 때, $$x+y$$가 `INT_MAX`보다 크거나 `INT_MIN`보다 작으면 오버플로가 발생한다.

음의 오버플로가 발생하려면 $$x$$, $$y$$ 모두 음수여야 하고, 양의 오버플로가 발생하려면 $$x$$, $$y$$ 모두 양수여야 한다.
...
`tadd_ok` 함수를 작성하는 연습문제도 글에 남겨본다. 오버플로가 발생하지 않은 경우에 1을 반환하는 함수다.
